...
describe("MovieDetails", () => {
  it("renders movie details correctly", () => {
    render(
      <MovieDetails
        movie={movie}
        movieDetailsList={["Poster", "opening_crawl", "director", "averageRating", "Ratings"]}
      />,
    );

    expect(screen.getByText(/Star Wars: A New Hope/i)).toBeInTheDocument();
    expect(screen.getByAltText(/Star Wars: A New Hope poster/i)).toBeInTheDocument();
    expect(screen.getByText(/A long time ago in a galaxy far, far away.../i)).toBeInTheDocument();

    expect(screen.getByText(/Average rating:/i)).toBeInTheDocument();
    expect(screen.getByText(/Internet Movie Database: 8.6\/10/i)).toBeInTheDocument();
